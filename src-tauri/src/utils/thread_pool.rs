...
pub(crate) type Task = dyn FnOnce() + Send + Sync;

/// 任务队列
pub(crate) struct TaskQueue(RwLock<Vec<Box<Task>>>, ThreadSignal);
...
impl<'a> TaskQueue {
    ...
    /// 获取任务数据
    /// timeout 间隔指定时间
    pub(crate) fn pop(&self, timeout: Duration) -> Option<Box<Task>> {
        // 读取任务
...
impl ThreadPool {
    ...
    /// max_time: 临时线程存活最大时间
    /// ``` 
    pub(crate) fn new(core_size: usize, max_size: usize, max_time: Duration) -> Self {
        Self {
            core_size: core_size,
    ...
    /// 提交任务
    /// ```
    pub(crate) fn submit(&self, task: Box<Task>) -> Result<(), SubmitError> {
        {
