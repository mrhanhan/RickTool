...
impl Display for ModuleInfo {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        f.write_fmt(format_args!(
            "Module: {} Desc: {} State: {:?}",
            meta.name, meta.desc, self.state
        ))
    }
}
...
impl ModuleManager {
    ...
    pub fn add_module(&self, module: Box<dyn Module>) {
        ...
        map.entry(module.meta().name)
            .or_insert(ModuleInfo::new(module));
    }
    /// 添加多个模块
    ...
    pub(crate) fn on_init(&self, app: Application) -> Result<(), ModuleError> {
        ...
        let stop = self.clone();
        app.event_context()
            .on_into(ApplicationEvent::Started, move |_app: &Application| {
                start.on_install(_app.clone());
            });
        app.event_context()
            .on_into(ApplicationEvent::Stoped, move |_app: &Application| {
                stop.close();
            });
