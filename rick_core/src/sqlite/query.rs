...
pub trait QueryDatabaseOperate {
    ...
    fn select_list_with_conn(
        wrapper: &SqlWrapper,
        conn: &Connection,
    ) -> Result<Vec<Self::Model>, SqlError> {
    ...
    fn select_by_id_with_conn<T: Into<SqlValue>>(
        value: T,
        conn: &Connection,
    ) -> Result<Option<Self::Model>, SqlError> {
    ...
    fn select_map_with_conn<K: Eq + Hash>(
        wrapper: &SqlWrapper,
        key_func: fn(&Self::Model) -> K,
        conn: &Connection,
    ) -> Result<HashMap<K, Self::Model>, SqlError> {
        let vec = Self::select_list_with_conn(wrapper, conn);
        match vec {
            Ok(_list) => {
                let mut map = HashMap::new();
                for i in _list {
                    map.entry(key_func(&i)).or_insert(i);
                }
                Ok(map)
            }
            Err(_err) => Err(_err),
