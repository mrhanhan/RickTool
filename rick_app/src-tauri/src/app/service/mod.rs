...
impl ServiceRegister {
    ...
    /// 注册闭包函数
    pub fn register_fn<
        F: Fn(ServiceInvoke, T) + Send + 'static,
        T: DeserializeOwned,
        O: Into<String>,
    >(
        &self,
        ...
        let _c = move |invoke: ServiceInvoke| {
            ...
            match data {
                ...
                Err(_err) => invoke.reject(ServiceResult::<i32>::fail_reason(
                    format!("参数错误:{}", _err).as_str(),
                )),
            }
        };
    ...
    pub fn register_closure_fn<M, E, F, T: DeserializeOwned, O: Into<String>>(
        &self,
        operate: O,
        func: F,
    ) where
        M: Serialize,
        E: RickError + 'static,
        F: (Fn(T) -> Result<M, E>) + Send + 'static,
    {
        let _c = move |invoke: ServiceInvoke| {
            let data = T::deserialize(invoke.data());
            match data {
                Ok(_data) => match func(_data) {
                    Ok(_result_data) => invoke.resolve(ServiceResult::success_data(_result_data)),
                    Err(_err) => {
                        invoke.reject(ServiceResult::<i32>::create(
                            _err.code(),
                            None,
                            _err.message().as_str(),
                        ));
    ...
    where
        M: Serialize,
        E: RickError + 'static,
        F: (Fn() -> Result<M, E>) + Send + 'static,
    {
        let _c = move |invoke: ServiceInvoke| match func() {
            Ok(_result_data) => invoke.resolve(ServiceResult::success_data(_result_data)),
            Err(_err) => {
                invoke.reject(ServiceResult::<i32>::create(
                    _err.code(),
    ...
    where
        M: Serialize,
        E: RickError + 'static,
        F: (Fn(&ServiceInvoke) -> Result<M, E>) + Send + 'static,
    {
        let _c = move |invoke: ServiceInvoke| match func(&invoke) {
            Ok(_result_data) => invoke.resolve(ServiceResult::success_data(_result_data)),
            Err(_err) => {
                invoke.reject(ServiceResult::<i32>::create(
                    _err.code(),
    ...
    /// 注册闭包函数
    pub fn register_unit_fn<M, E, F, T: DeserializeOwned, O: Into<String>>(
        &self,
        operate: O,
        func: F,
    ) where
        M: Serialize,
        E: RickError + 'static,
        F: (Fn(&ServiceInvoke, T) -> Result<M, E>) + Send + 'static,
    {
        let _c = move |invoke: ServiceInvoke| {
            let data = T::deserialize(invoke.data());
            match data {
                Ok(_data) => match func(&invoke, _data) {
                    Ok(_result_data) => invoke.resolve(ServiceResult::success_data(_result_data)),
                    Err(_err) => {
                        invoke.reject(ServiceResult::<i32>::create(
                            _err.code(),
        ...
        app.thread_pool()
            .submit(Box::new(move || {
                ...
                match result {
                    ...
                    Ok(Err(_err)) => {
                        resolver.reject(ServiceResult::<i32>::create(
                            _err.code(),
                            None,
                            _err.message().as_str(),
                        ));
                    ...
                    Err(ref _err) => {
                        if let Some(s) = _err.downcast_ref::<&str>() {
                            resolver.reject(ServiceResult::<i32>::fail_reason(*s));
                        } else if let Some(s) = _err.downcast_ref::<String>() {
                            resolver.reject(ServiceResult::<i32>::fail_reason(s.as_str()));
                        } else if let Some(s) = _err.downcast_ref::<SqlError>() {
                            if let Some(ref message) = s.message {
                                resolver.reject(ServiceResult::<i32>::fail_reason(message.as_str()));
                            } else {
                                resolver.reject(ServiceResult::<i32>::fail_reason("Sqlite 错误"));
                            }
                        } else {
                            resolver.reject(ServiceResult::<i32>::fail_reason("Error"));
                        }
